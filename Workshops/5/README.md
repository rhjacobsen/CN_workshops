...
It is possible to store persistent quagga configuration files. First you need to add the /etc/quagga directory part of the router configuration. Right-click on the router icon and choose configure. In the [Advance tab you add the directory](imgs/persistent_quagga_files.png) to additional directories. Then you use the command ```write file``` in the VTY.
